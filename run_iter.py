...
def get_optimal_control(steer_ref,steer_var,v,theta,theta_var,x,x_var,curvature,curvature_var,v_perp=0.,omega=0.) :
    # theta = -theta
    ...
    if VEHICLE_MODEL=='kinematic' :
        ...
        for steer in np.arange(-MAX_STEER,MAX_STEER,0.01) :
            ...
            var_left = math.sqrt((abs(lambda_**2*x_var))**2 + (v**2*math.sin(theta)*steer/L)**2 + (v**2*curvature_var)**2 + (lambda_*(v*math.cos(theta))*theta_var)**2)
            var_right = math.sqrt((abs(lambda_**2*x_var))**2 + (v**2*math.sin(theta)*steer/L)**2 + (v**2*curvature_var)**2 + (lambda_*(v*math.cos(theta))*theta_var)**2)
            if (hddd_left + lambda_*hdd_left + lambda_**2*hd_left+lambda_**3*h_left-var_left*norm.ppf(1-BETA)) < 0 :
                print("Right violation") 
                cost += alpha*(hddd_left + lambda_*hdd_left + lambda_**2*hd_left+lambda_**3*h_left-var_left*norm.ppf(1-BETA))**2
            if (hddd_right+lambda_*hdd_right+lambda_**2*hd_right+lambda_**3*h_right-var_right*norm.ppf(1-BETA)) < 0 :
                print("Left violation")
                cost += alpha*(hddd_right+lambda_*hdd_right+lambda_**2*hd_right+lambda_**3*h_right-var_right*norm.ppf(1-BETA))**2
            if cost < min_cost :
                min_cost = cost
                min_steer = steer
    return min_steer
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for frame in tqdm.tqdm(range(TOTAL_EPISODE_FRAMES)):
            ...
            vel_x_num = (current_x-prev_x)/(float(measurement_data.game_timestamp) / 1000.0 - current_timestamp)
            vel_y_num = (current_y-prev_y)/(float(measurement_data.game_timestamp) / 1000.0 - current_timestamp)
            ...
            if VEHICLE_MODEL=='kinematic' :
                ...
                current_yaw_updated = current_yaw+math.atan2(current_speed_perp,current_speed)
